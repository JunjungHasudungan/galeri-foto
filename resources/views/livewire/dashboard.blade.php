<div>
    <div>
        @if ($is_create)
            @include('livewire.pages.dashboard._modalCreatePost')
        @endif
    </div>
    @forelse ($userLike as $like)
        <div class="w-full p-4 mb-2 text-start bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-row">
                <img class="object-cover w-45 rounded-lg h-25 md:h-25 md:w-48" src="{{ asset('/assets/media/product/books/4.png')}}" alt="">
                <div class="px-2 w-full  flex-col">
                    <div class="space-y-4 mt-2">
                        <a href="{{ route('dashboard') }}" class="mb-8 p-3">
                            @php
                                $user = \App\Models\User::select('name')->where('id', $like->post->user_id)->first();
                            @endphp
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ $user->name }} . {{ \Carbon\Carbon::parse($like->post->created_at)->translatedFormat('j M') }} </p>

                            <h5 class="text-sm tracking-tight text-semibold text-gray-400"> {{ $like->post->keterangan }} </h5>
                        </a>
                    </div>
                </div>
            </div>
            <div class="mt-3 space-x-4 flex flex-row items-start self-end">
                ...
                <div class="flex flex-row space-x-1 p-2">
                    <button type="button" class="text-blue-700 focus:outline-none focus:ring-blue-300 rounded-lg hover:red-700 hover:text-white font-medium text-xs p-2 text-center inline-flex items-center  dark:text-blue-500 dark:hover:text-white  dark:hover:bg-blue-400">
                        ...
                        <label class="text-base item-center self-center"> {{ count($like->post->likes) ?? 0}} </label>
                </div>
            </div>
        </div>
    @empty
