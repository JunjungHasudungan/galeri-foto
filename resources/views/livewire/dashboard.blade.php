<div>
    <div>
        @if ($is_create)
            @include('livewire.pages.dashboard._modalCreatePostComment')
        @endif
    </div>
        ...
        <div class="w-full p-4 mb-2 text-start bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="space-y-4 mt-2">
                        <a href="{{ route('dashboard') }}" class="mb-8 p-3">
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
                                {{ $post->user->name }} 
                                . {{ \Carbon\Carbon::parse($post->created_at)->translatedFormat('j M') }}
                            </p>
            ...
            <div class="mt-3 space-x-4 flex flex-row items-start self-end">
                <div class="flex flex-row space-x-1 p-2">
                    <button wire:click.prevent="createCommentPost({{$post->id}})"
                ...
                <div class="flex flex-row space-x-1 p-2">
                    <button 
                        type="button" 
                        wire:click.prevent="storeLikePost({{ $post->id }})"
